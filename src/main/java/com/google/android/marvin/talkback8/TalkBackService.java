...
import com.android.talkback.controller.SoundSwitch;
import com.czy.virtual.VirtualScreen;
import com.czy.ocr.ScreenShot;
...
import com.xinyang.screenreader.R;
import com.android.talkback.NetBroadcastReceiver;
...
public class TalkBackService extends AccessibilityService
    ...
    public static final String PERMISSION_TALKBACK =
            "com.xinyang.screenreader.feedback.permission.TALKBACK";
    ...
    private static final String ACTION_RESUME_FEEDBACK =
            "com.xinyang.screenreader.RESUME_FEEDBACK";
...
return mProcessorPhoneticLetters;
}
...
// try to register screenshot listener
mScreenShot.addContentObserver(this);

// started mta
	try {
StatService.setContext(this.getApplication());
StatConfig.initNativeCrashReport(this.getApplication(), null);
StatService.startStatService(this.getApplication(), null,
...
//XGPushManager.registerPush(this.getApplication()); 

//Intent service = new Intent(context, XGPushService.class);  
//context.startService(service);  

//
 // registerPush(context,account) or registerPush(context,account, XGIOperateCallback)
...
// try to unregister screenshot listener
mScreenShot.removeContentObserver(this);
    }
...
if( mVirtualScreen.getOpening() ) {

} else {
if(NimbleMenu.isOpening && event.getEventType() == AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED) {
mSpeechController.interrupt();
mAccessibilityEventProcessor.onAccessibilityEvent(event);
NimbleMenu.getTextProcess(event);
} else {
        mAccessibilityEventProcessor.onAccessibilityEvent(event);
}
}
...
if(mVirtualScreen.getVirtualScreenListener()!=null && mVirtualScreen.getVirtualScreenListener().onGesture(gestureId) ) {
return true;
}
...
NimbleMenu.setNimbleMenu(this);

final Compared compared = new Compared();
int result = compared.compare(this);
if(result<=0){
suspendTalkBack();
stopSelf();
return;
...
int type = compared.checkType(this, false);
...
@Override
public void run() {
compared.updateType(TalkBackService.this, true, true);
}
});
} else {
...
switch(type) {
case 1: // special
FailoverTextToSpeech.getSynthProxy(this).speak(getString(R.string.open_welcome_words), "czy");
break;
case 2: // vip
FailoverTextToSpeech.getSynthProxy(this).speak(getString(R.string.open_welcome_words_vip), "czy");
break;
default:
FailoverTextToSpeech.getSynthProxy(this).speak(getString(R.string.talkback_on), "czy");
break;
}
            }
        }
...
netBroadcastReceiver.setNetEvent(new NetUtil.NetEvent() {
@Override
...
/**
 * if vip expired, but user.getType= 2
 * check vip from service data
*/
public void regainVipType() {
final Handler handler = new Handler();
new Thread(new Runnable() {
@Override
public void run() {
User user = new User();
Compared compared = new Compared();
int type = compared.checkType(TalkBackService.this, true);
if( user.is() && user.getType()==2 && type!=2 ) {
compared.gainVipInfo(TalkBackService.this, user.getId(), user.getPhone(), user.getIdCard(), user.getToken(),
...
@Override
public void run() {
Toast.makeText(TalkBackService.this, R.string.toast_connect_internet_failed, Toast.LENGTH_SHORT).show();
}
});
}
});
...
/**
 * set the alarm times to increase vip updatetimes
*/
public void startFailedUpdateTimesAlarm() {
Intent intent = new Intent("FAILED_UPDATE_TIMES_ELITOR_CLOCK");  
PendingIntent pi = PendingIntent.getBroadcast(this,0,intent,0);
AlarmManager am = (AlarmManager)getSystemService(ALARM_SERVICE);    
am.setRepeating(AlarmManager.RTC, System.currentTimeMillis() + (5*60*1000), 60*60*1000, pi);
}
...
mProcessorPhoneticLetters = new ProcessorPhoneticLetters(this, mSpeechController);
        addEventListener(mProcessorPhoneticLetters);
...
String tempSpeakScreenoff = SharedPreferencesUtils.getStringPref(mPrefs, res, R.string.pref_speak_screenoff_key, R.string.pref_speak_screenoff_default);
if(tempSpeakScreenoff!=null) {
if(tempSpeakScreenoff.equals("readAll")) {
        mAccessibilityEventProcessor.setSpeakWhenScreenOff(0);
}else if(tempSpeakScreenoff.equals("read")) {
        mAccessibilityEventProcessor.setSpeakWhenScreenOff(1);
}else{
        mAccessibilityEventProcessor.setSpeakWhenScreenOff(2);
}
}else{
mAccessibilityEventProcessor.setSpeakWhenScreenOff(0);
}
...
CursorControllerApp.mShouldScroll = mPrefs.getBoolean( getString(R.string.pref_auto_scroll_key), true);
FailoverTextToSpeech.mWhichTtsEngine = mPrefs.getBoolean(getString(R.string.pref_tts_engine_key), true);
mSpeechController.readPunctuation = mPrefs.getBoolean(getString(R.string.pref_punctuation_key), false);
ProcessorScrollPosition.mReadScroll = mPrefs.getBoolean(getString(R.string.pref_read_list_key), true);
if(SoundSwitch.update(mPrefs, getString(R.string.pref_sound_sort_key)) && mFeedbackController!=null) {
mFeedbackController.reload();
}
if(mRingerModeAndScreenMonitor!=null) {
mRingerModeAndScreenMonitor.setSort(mPrefs.getString(getString(R.string.pref_open_screen_read_key), "readAll"));
}
updateTranslatePrefs();
updateTouchSortPrefs();
if(mVolumeMonitor!=null) {
mVolumeMonitor.setVolumeSpeak(mPrefs.getBoolean(getString(R.string.pref_volume_speak_key), false));
}
    }

private void updateTouchSortPrefs() {
ProcessorFocusAndSingleTap.touchSort = mPrefs.getBoolean(getString(R.string.pref_touch_sort_key), false);
Compared compared = new Compared();
if(ProcessorFocusAndSingleTap.touchSort && compared.checkType(this, false) <= 0 ) {
Toast.makeText(this, getString(R.string.notify_change_touch_sort_disable), Toast.LENGTH_SHORT).show();				
...
private void updateTranslatePrefs() {
BDTranslate.opening = mPrefs.getBoolean(getString(R.string.pref_translate_key), false);
Compared compared = new Compared();
if(BDTranslate.opening && compared.checkType(this, false) <= 0 ) {
Toast.makeText(this, getString(R.string.tip_translate_need), Toast.LENGTH_SHORT).show();				
...
//android.util.Log.i("czys",  "web: " +requestedState);
        final boolean currentState = (
                (info.flags & AccessibilityServiceInfo.FLAG_REQUEST_ENHANCED_WEB_ACCESSIBILITY)
                != 0);
        if (currentState == requestedState) {
...
if(key.equals(getString(R.string.pref_tts_engine_key))) {
if(mSpeechController!=null) {
mSpeechController.interrupt();
}
FailoverTextToSpeech.mWhichTtsEngine = prefs.getBoolean(key, true);
}else if(key.equals(getString(R.string.pref_read_list_key))) {
ProcessorScrollPosition.mReadScroll = prefs.getBoolean(key, true);
}else if(key.equals(getString(R.string.pref_touch_sort_key))) {
updateTouchSortPrefs();
}else if(key.equals(getString(R.string.pref_open_screen_read_key))) {
if(mRingerModeAndScreenMonitor!=null) {
mRingerModeAndScreenMonitor.setSort(mPrefs.getString(getString(R.string.pref_open_screen_read_key), "readAll"));
}
}else if(key.equals(getString(R.string.pref_translate_key))) {
 updateTranslatePrefs();
}else if(key.equals(getString(R.string.pref_punctuation_key))) {
mSpeechController.readPunctuation = prefs.getBoolean(key, false);
}else if(key.equals(getString(R.string.pref_sound_sort_key))) {
if(SoundSwitch.update(prefs, key) && mFeedbackController!=null) {
mFeedbackController.reload();
}
}else if(key.equals(getString(R.string.pref_volume_speak_key))) {
if(mVolumeMonitor!=null) {
mVolumeMonitor.setVolumeSpeak(mPrefs.getBoolean(getString(R.string.pref_volume_speak_key), false));
}
} else {
