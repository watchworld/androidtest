...
public class FailoverTextToSpeech {
    ...
    private static SynthProxy mSynthProxy = null;
// true is used aisound
public static boolean mWhichTtsEngine = true;

public static SynthProxy getSynthProxy(Context context) {
if( mSynthProxy==null ) {
mSynthProxy = new SynthProxy(context, context.getAssets(), "zh");
}
return mSynthProxy;
}
...
getSynthProxy(context);
if(mSynthProxy!=null) {
mSynthProxy.setOnSpeakCompletedListener(new SynthProxy.OnSpeakCompletedListener() {
@Override
...
FailoverTtsListener ln = mListeners.get(i);
if(ln!=null && ln.getClass().getName().equals(listener.getClass().getName())) {
mListeners.remove(i);
break;
}
}
...
mSynthProxy.setOnSpeakCompletedListener(null);
mSynthProxy.shutdown();
mSynthProxy = null;
    }
...
if(mWhichTtsEngine) {
mSynthProxy.speak(text.toString(), utteranceId);
