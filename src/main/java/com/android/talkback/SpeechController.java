...
private Map<String,String> mappunctuations = new HashMap<String,String>();

public boolean readPunctuation = false;
...
//
InputStream stream = context.getResources().openRawResource(R.raw.punctuation);
...
JSONArray punctuations = new JSONArray(stringBuilder.toString());

for(int i=0; i < punctuations.length() ; i++) {
JSONObject o = (JSONObject) punctuations.get(i);
Iterator<?> keys = o.keys();
while(keys.hasNext()) {
String letter = (String) keys.next();
mappunctuations.put(letter, o.getString(letter));
                    }
} // for
...
if(readPunctuation) {
for(Map.Entry<String,String> entry : mappunctuations.entrySet()) {
text = text.toString().replace( entry.getKey(), entry.getValue());
}
}
...
public void addListener(FailoverTextToSpeech.FailoverTtsListener listener) {
mFailoverTts.addListener(listener);
}
public void removeListener(FailoverTextToSpeech.FailoverTtsListener listener) {
mFailoverTts.removeListener(listener);
}
