...
import com.czy.settings.BackupSettingActivity;
...
//            }
    ...
    /**
     * Assigns the appropriate intent to the tts engine preference.
     */
private void assignTtsEngineIntent() {
        final Preference prefManageTtsEngine = findPreferenceByResId(R.string.pref_category_manage_tts_engine_key);
        if(prefManageTtsEngine == null) {
            return;
        }
        final Intent ttsEngineIntent = new Intent(getActivity(), AisoundActivity.class);
        ttsEngineIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ttsEngineIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        prefManageTtsEngine.setIntent(ttsEngineIntent);
    }

    /**
     * Assigns the appropriate intent to the backup setting preference.
     */
private void assignBackupSettingIntent() {
        final Preference prefBackupSetting = findPreferenceByResId(R.string.pref_backup_setting_key);
        if(prefBackupSetting == null) {
            return;
        }
        final Intent backupSettingIntent = new Intent(getActivity(), BackupSettingActivity.class);
        backupSettingIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        backupSettingIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        prefBackupSetting.setIntent(backupSettingIntent);
    }
...
private void updateTranslatePreference() {
/*
final CheckBoxPreference prefTranslate = (CheckBoxPreference) findPreferenceByResId(R.string.pref_translate_key);

            if (prefTranslate == null || mPrefs == null) {
                return;
            }

prefTranslate.setChecked(mPrefs.getBoolean(getString(R.string.pref_translate_key), false));
*/
        }
                ...
                new OnSharedPreferenceChangeListener() {
                    ...
                    public void onSharedPreferenceChanged(SharedPreferences sharedPrefs, String key) {
                        ...
                        if (key != null && key.equals(dimKey)) {
                            updateDimingPreferenceStatus();
                        } else if(key!= null && key.equals(getString(R.string.pref_translate_key))) {
updateTranslatePreference();
}
                    }
                };
