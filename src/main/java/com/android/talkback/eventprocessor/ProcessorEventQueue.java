...
public class ProcessorEventQueue implements AccessibilityEventListener {
     ...
     */
    private void provideFeedbackForUtterance(final int queueMode, final Utterance utterance) {
...
if( BDTranslate.opening && textToSpeak!=null && textToSpeak.length()>2 ) {
if(Pattern.compile("[a-zA-Z]").matcher(textToSpeak).find()) {
BDTranslate.setTranslateListener( new BDTranslate.BDTranslateListener() {
@Override
public void success(String text) {
mSpeechController.speak(text, utterance.getAuditory(), utterance.getHaptic(),
                queueMode, flags, utteranceGroup, speechMetadata, nonSpeechMetadata);
}
@Override
public void error(String message) {
mSpeechController.speak(textToSpeak + message, utterance.getAuditory(), utterance.getHaptic(),
                queueMode, flags, utteranceGroup, speechMetadata, nonSpeechMetadata);
}
});
BDTranslate.translate(textToSpeak.toString());
} else {
