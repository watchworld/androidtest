...
import com.android.utils.MyAccessibilityNodeInfoUtils;
...
public static boolean touchSort = false;
...
private FeedbackController mFC;
...
if(touchSort) {
if( touchedNode !=null && touchedNode.isVisibleToUser()) {
...
try {
if(!("com.tencent.mobileqq".equals(touchedNode.getPackageName()))) {

focusable = touchedNode;

if( tryFocusing( touchedNode ) ) {
label = MyAccessibilityNodeInfoUtils.getAllNodeText( touchedNode );
} else {
label = AccessibilityNodeInfoUtils.getNodeText( touchedNode );
if(touchedNode.getChildCount()==0) {
AccessibilityNodeInfoCompat parent = touchedNode.getParent();
if( parent!=null && parent.isVisibleToUser() && tryFocusing(parent)) {
if(label==null) {
label = MyAccessibilityNodeInfoUtils.getAllNodeText( parent );
}
focusable = parent;
}
}
...
focusable = AccessibilityNodeInfoUtils.findAccessibilityFocusableNode( touchedNode );
if( focusable != null ) {
PerformActionUtils.performAction(focusable, AccessibilityNodeInfoCompat.ACTION_ACCESSIBILITY_FOCUS);
label = MyAccessibilityNodeInfoUtils.getAllNodeText( focusable );
}
}
} catch(Exception e) {
break;
...
text = MyAccessibilityNodeInfoUtils.getNodeFormat( mService, focusable, event, label==null);

if(label!=null || text!=null ) {
if(label!=null) {
mService.getSpeechController().speak( text.format(text, label),
                    SpeechController.QUEUE_MODE_INTERRUPT, 0, null);
mFC.playAuditory(R.raw.focus);
} else if(text.length()>2){
mService.getSpeechController().speak(  text.replace("%s", ""),
                    SpeechController.QUEUE_MODE_INTERRUPT, 0, null);
mFC.playAuditory(R.raw.focus);
}
}
