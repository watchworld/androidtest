...
result = mCursorController.previous(true /* shouldWrap */,
                    true /* shouldScroll */,
...
boolean result = false;
if( ProcessorFocusAndSingleTap.touchSort ) {
result = mCursorController.next();
} else {
result = mCursorController.next(true /* shouldWrap */,
                    true /* shouldScroll */,
                    true /*useInputFocusAsPivotIfEmpty*/,
                    InputModeManager.INPUT_MODE_TOUCH);
}
...
mFeedbackController.playAuditory(R.raw.back);
NimbleMenu.close();
...
} else if (action.equals(mService.getString(R.string.shortcut_value_quickly_breakout))) {
NimbleMenu.showPopupWindow();
        } else if (action.equals(mService.getString(R.string.shortcut_value_change_touch_sort))) {
SpeechController sc = mService.getSpeechController();
if(sc!=null) {
Compared compared = new Compared();
if( compared.checkType(mService, true) > 0 ) {
if(!ProcessorFocusAndSingleTap.touchSort) {
sc.speak(mService.getString(R.string.notify_change_touch_sort_more), SpeechController.QUEUE_MODE_INTERRUPT, 0, null);
ProcessorFocusAndSingleTap.touchSort = true;
} else {
sc.speak(mService.getString(R.string.notify_change_touch_sort_normal), SpeechController.QUEUE_MODE_INTERRUPT, 0, null);
ProcessorFocusAndSingleTap.touchSort = false;
}
SharedPreferences prefs = SharedPreferencesUtils.getSharedPreferences(mService);
Editor editor = prefs.edit();
editor.putBoolean(mService.getString(R.string.pref_touch_sort_key), ProcessorFocusAndSingleTap.touchSort);
editor.commit();
} else {
sc.speak(mService.getString(R.string.notify_change_touch_sort_disable), SpeechController.QUEUE_MODE_INTERRUPT, 0, null);
}
}
